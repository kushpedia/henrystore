...
class ReturnEmailService:
    ...
    @staticmethod
    def send_paid_order_admin_notification(order):
        """Send email to admin when a new order is paid"""
        try:
            subject = f"🛒 Order - #{order.oid} Paid Successfully"

            order_items = order.cartorderitems_set.all()  # adjust if related_name exists
            domain = getattr(settings, 'SITE_DOMAIN', 'localhost:8080')
            protocol = getattr(settings, 'SITE_PROTOCOL', 'https')
            html_content = render_to_string(
                'core/emails/admin_pai_order_notification.html',
                {
                    'order': order,
                    'order_items': order_items,
                    'customer': order.user,
